...
def create_demo_data():
    with app.app_context():
        ...
        tx1 = Transaction(account_from_id=acc1.id, account_to_id=acc2.id, type="Которуу", amount=1500, timestamp=datetime.utcnow(), description="Айзада -> Бакыт (Которуу)")
        tx2 = Transaction(account_from_id=acc1.id, account_to_id=acc2.id, type="Толуктоо", amount=1500, timestamp=datetime.utcnow(), description="Айзададан алынган каражат")
        tx3 = Transaction(account_from_id=None, account_to_id=acc3.id, type="Толуктоо", amount=2000, timestamp=datetime.utcnow(), description="Айлык маяна")
        tx4 = Transaction(account_from_id=acc4.id, account_to_id=None, type="Чыгым", amount=500, timestamp=datetime.utcnow(), description="Банкоматтан алуу")
        tx5 = Transaction(account_from_id=acc2.id, account_to_id=acc3.id, type="Которуу", amount=1000, timestamp=datetime.utcnow(), description="Бакыт -> Чыңгыз (Которуу)")
